  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card p-6 md:p-8 rounded-2xl border border-slate-700/60">
        ...
        <h1 className="text-2xl font-extrabold text-black">{t.simulator.title}</h1>
        <p className="text-sm text-slate-700 mt-1 max-w-2xl">{t.simulator.subtitle}</p>
      </div>
      ...
      <div className="glass-card p-6 rounded-2xl border border-slate-700/60 grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        {/* Number of Partners */}
        <div className="space-y-2">
          <label htmlFor={partnersId} className="text-xs font-semibold text-slate-700 flex items-center space-x-1">
        ...
        {/* Growth Target */}
        <div className="space-y-2">
          <label htmlFor={growthId} className="text-xs font-semibold text-slate-700 flex items-center space-x-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className={`glass-card p-6 rounded-2xl border transition-all relative ${
          ...
          <h3 className="text-lg font-bold text-black text-center mt-2">{t.simulator.solePropTitle}</h3>
          <p className="text-xs text-slate-600 text-center mt-1">Simplest for single entrepreneurs</p>

          <div className="mt-6 space-y-4 text-xs">
            <div className="p-3 rounded-xl bg-slate-100 border border-slate-300">
              <span className="text-slate-600 block">{t.simulator.taxRateLabel}</span>
              <span className="font-bold text-slate-900 block mt-0.5">{t.simulator.taxRateSole}</span>
              <span className="text-sm font-bold text-emerald-700 font-mono block mt-2">
                Est. Tax: ৳ {Math.round(solePropTax).toLocaleString('en-IN')}
              </span>
            </div>

            <div className="flex justify-between items-center border-b border-slate-200 pb-2">
              <span className="text-slate-600">{t.simulator.registrationCostLabel}</span>
              <span className="font-bold text-black font-mono">৳ {solePropSetupCost.toLocaleString('en-IN')}</span>
            </div>

            <div className="flex justify-between items-center border-b border-slate-200 pb-2">
              <span className="text-slate-600">{t.simulator.complianceCostLabel}</span>
              <span className="font-bold text-black font-mono">৳ {solePropComplianceCost.toLocaleString('en-IN')}</span>
            </div>

            <div className="space-y-1.5">
              <span className="text-slate-600 block">{t.simulator.liabilityLabel}</span>
              <span className="font-bold text-amber-700 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/30 inline-block">
        ...
        <div className={`glass-card p-6 rounded-2xl border transition-all relative ${
          ...
          <h3 className="text-lg font-bold text-black text-center mt-2">{t.simulator.partnershipTitle}</h3>
          <p className="text-xs text-slate-600 text-center mt-1">Best for co-founded traditional firms</p>

          <div className="mt-6 space-y-4 text-xs">
            <div className="p-3 rounded-xl bg-slate-100 border border-slate-300">
              <span className="text-slate-600 block">{t.simulator.taxRateLabel}</span>
              <span className="font-bold text-slate-900 block mt-0.5">{t.simulator.taxRatePartner}</span>
              <span className="text-sm font-bold text-emerald-700 font-mono block mt-2">
            ...
            <div className="flex justify-between items-center border-b border-slate-200 pb-2">
              <span className="text-slate-600">{t.simulator.registrationCostLabel}</span>
              <span className="font-bold text-black font-mono">৳ {partnershipSetupCost.toLocaleString('en-IN')}</span>
            </div>

            <div className="flex justify-between items-center border-b border-slate-200 pb-2">
              <span className="text-slate-600">{t.simulator.complianceCostLabel}</span>
              <span className="font-bold text-black font-mono">৳ {partnershipComplianceCost.toLocaleString('en-IN')}</span>
            </div>
        ...
        <div className={`glass-card p-6 rounded-2xl border transition-all relative ${
          ...
          <h3 className="text-lg font-bold text-black text-center mt-2">{t.simulator.llcTitle}</h3>
          <p className="text-xs text-slate-600 text-center mt-1">Best for high growth & investor funding</p>

          <div className="mt-6 space-y-4 text-xs">
            <div className="p-3 rounded-xl bg-slate-100 border border-slate-300">
              <span className="text-slate-600 block">{t.simulator.taxRateLabel}</span>
              <span className="font-bold text-slate-900 block mt-0.5">{t.simulator.taxRateLLC}</span>
              <span className="text-sm font-bold text-emerald-700 font-mono block mt-2">
            ...
            <div className="flex justify-between items-center border-b border-slate-800 pb-2">
              <span className="text-slate-600">{t.simulator.registrationCostLabel}</span>
              <span className="font-bold text-black font-mono">৳ {llcSetupCost.toLocaleString('en-IN')}</span>
            </div>

            <div className="flex justify-between items-center border-b border-slate-800 pb-2">
              <span className="text-slate-600">{t.simulator.complianceCostLabel}</span>
              <span className="font-bold text-black font-mono">৳ {llcComplianceCost.toLocaleString('en-IN')}</span>
            </div>
